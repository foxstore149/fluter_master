...
function getApiBase() {
  ...
  return "https://flutter-master-api.foridur06.workers.dev";
}
  ...
  checkAuth().then(async function (user) {
    if (!user) {
      ...
      try {
        ...
        window.__brand = {
          name: asStr(d.brand_name, "Flutter Master") || "Flutter Master",
          logoUrl: asStr(d.brand_logo_url, "") || ""
        };
      } catch (e) {
        window.__brand = window.__brand || { name: "Flutter Master", logoUrl: "" };
      }
...
function mountShellHtml(active) {
  ...
  var brand = window.__brand || { name: "Flutter Master", logoUrl: "" };
  var brandName = brand.name || "Flutter Master";
